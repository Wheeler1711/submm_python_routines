        ...
        while found_spacing_conflict:
            ...
            if number_of_minima_this_region > 1:
                for counter in range(number_of_minima_this_region - 1):
                    ...
                    minima_spacing_Hz = abs(self.f_Hz[data_index_minima_left_test] - self.f_Hz[data_index_minima_right_test])
...
def find_vna_sweep(f_Hz, z, smoothing_scale_Hz=5.0e6, spacing_threshold_Hz=1.0e5):
    ...
    ipt = interactive_threshold_plot(f_Hz=f_Hz,
    ...
    ip = interactive_plot(f_Hz, highpass_mags, kid_idx)
    return ip
