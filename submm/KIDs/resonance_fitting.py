...
def cardan(a, b, c, d):
    ...
    if D > 0:
        ...
        return np.real(roots[np.argsort(np.abs(np.imag(roots)))][
                           0])  # one real root get the value that has smallest imaginary component
...
def nonlinear_mag(x, fr, Qr, amp, phi, a, b0, b1, flin):
    """
    x is the frequencies in your iq sweep covers
    fr is the center frequency of the resonator
    Qr is the quality factor of the resonator
    amp is Qr/Qc
    phi is a rotation parameter for an impedance mismatch between the resonator and the readout system
    a is the non-linearity parameter bifurcation occurs at a = 0.77
    ...
    z = (b0 + b1 * xlin) * np.abs(
        1.0 - amp * np.exp(1.0j * phi) / (1.0 + 2.0 * 1.0j * y) + amp / 2. * (np.exp(1.0j * phi) - 1.0)) ** 2
    return z
...
def fit_nonlinear_iq(x, z, verbose=True, **keywords):
    ...
    x0    --- initial guess for the fit this can be very important because least square space over all
              parameters is not completely searched.
    amp_norm --- do a normalization for variable amplitude. This parameter is useful when the
                 transfer function of the cryostat is not flat
    tau forces tau to specific value
    tau_guess fixes the guess for tau without have to specify all of x0
    """
    if 'tau' in keywords:
        use_given_tau = True
        tau = keywords['tau']
...
def fit_nonlinear_iq_sep(fine_x, fine_z, gain_x, gain_z, **keywords):
    ...
    if 'fine_z_err' in keywords and 'gain_z_err' in keywords:
        use_err = True
...
def fit_nonlinear_iq_with_err(x, z, **keywords):
    ...
    if 'bounds' in keywords:
        ...
        # define default bounds
        print("default bounds used")
        bounds = ([np.min(x), 2000, .01, -np.pi, 0, -5, -5, 1 * 10 ** -9, np.min(x)],
                  [np.max(x), 200000, 1, np.pi, 5, 5, 5, 1 * 10 ** -6, np.max(x)])
    if 'x0' in keywords:
        x0 = keywords['x0']
    else:
        # define default intial guess
...
def fit_nonlinear_mag(x, z, verbose=True, **keywords):
    ...
    if 'bounds' in keywords:
        ...
        # define default bounds
        print("default bounds used")
        bounds = ([np.min(x), 100, .01, -np.pi, 0, -np.inf, -np.inf, np.min(x)],
                  [np.max(x), 200000, 1, np.pi, 5, np.inf, np.inf, np.max(x)])
    if 'x0' in keywords:
        x0 = keywords['x0']
    else:
        # define default intial guess
