...
class interactive_power_tuning_plot(object):
    ...
    def __init__(self,f,z,attn_levels,fitted_a_mag = None,fitted_a_iq = None,desired_a = 0.5,z_fit_mag = None,z_fit_iq = None):
        ...
        for i in range(0,self.f.shape[1]):
            if fitted_a_mag is not None:
                ...
                if first_bif_mag == 0:
                    first_bif_mag =1
                interp_mag  = interpolate.interp1d(-self.attn_levels[0:first_bif_mag+1],fitted_a_mag[i,:][0:first_bif_mag+1])
                powers_mag = np.linspace(-self.attn_levels[0]+.01,-self.attn_levels[first_bif_mag]-0.01,1000)
                bifurcation_mag = powers_mag[np.argmin(np.abs(interp_mag(powers_mag)-np.ones(len(powers_mag))*self.bif_level))]
            ...
            if fitted_a_iq is not None:
                ...
                if first_bif_iq == 0:
                    first_bif_iq = 1
                interp_iq  = interpolate.interp1d(-self.attn_levels[0:first_bif_iq+1],fitted_a_iq[i,:][0:first_bif_iq+1] )
                powers_iq = np.linspace(-self.attn_levels[0]+.01,-self.attn_levels[first_bif_iq]-0.01,1000)
                bifurcation_iq = powers_iq[np.argmin(np.abs(interp_iq(powers_iq)-np.ones(len(powers_iq))*self.bif_level))]
        ...
        if z_fit_mag is not None:
            self.l1_fit, = self.ax.plot(self.chan_freqs[:,self.plot_index,self.power_index],
                                        10*np.log10(self.z_fit_mag[:,self.plot_index,self.power_index]),label = "Fit")
        self.l1, = self.ax.plot(self.chan_freqs[:,self.plot_index,self.power_index],
                                20*np.log10(np.abs(self.z[:,self.plot_index,self.power_index])),'o',mec = "k",label = "Data")
        self.ax.legend()
        if self.z_fit_iq is not None:
        ...
        self.ax2.legend()
        if self.fitted_a_mag is not None:
            self.l3, = self.ax3.plot(-self.attn_levels,self.fitted_a_mag[self.plot_index,:],color = 'r',label = "Mag fit")
    ...
    def refresh_plot(self):
        self.l1.set_data(self.chan_freqs[:,self.plot_index,self.power_index],
                         10*np.log10(self.Is[:,self.plot_index,self.power_index]**2+self.Qs[:,self.plot_index,self.power_index]**2))
...
def tune_resonance_power(f,z,attn_levels,fitted_a_mag = None,fitted_a_iq = None,desired_a = 0.5,z_fit_mag = None,z_fit_iq = None):
    ...
    normalizing_amplitudes = np.sqrt(10**((ip.bif_levels+attn_levels[-1])/10)/np.min(10**((ip.bif_levels+attn_levels[-1])/10)))
    plt.subplot(212)
