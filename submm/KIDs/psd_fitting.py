...
def noise_white_rolloff(y,a,d):
    return (a)/(1+(2*np.pi*y*d)**2.)
...
def noise_profile_lor_vec(y,a,b,c,d):
    ...
    return (a+b*y**-c)/(1+(2*np.pi*y*d)**2.)
def noise_profile_lor_TLS(y,a,b,c,d,e):
    return (a/(1+(2*np.pi*y*d)**2.)+b*y**-c)*(1/(1+(2*np.pi*y*e)**2.))
def noise_profile_lor_TLS_vec(y,a,b,c,d,e):
    y = np.reshape(y,(y.shape[0],1,1,1,1,1))
    return (a/(1+(2*np.pi*y*d)**2.)+b*y**-c)*(1/(1+(2*np.pi*y*e)**2.))
...
def fit_psd_lor_brute(x, y, n_grid_points=20, error=None, **keywords):
    ...
    if ('ranges' in keywords):
        ...
        print("guess values are")
        print(x0_guess)
        ranges = np.asarray(([x0_guess[0] / 2, x0_guess[1] / 80, 0.4, x0_guess[3] / 2],
...
def fit_psd_lor_TLS_brute(x, y, tau_Q,n_grid_points=20, error=None, **keywords):
    """
    brute force fitting (the only way to fit with 4 or less variables)
    x is the psd frequency
    y is the psd magnitude
    ...
    if not slope:
        ...
        a, b, c, d = np.meshgrid(a_values, b_values, c_values, d_values, indexing="ij")  # always index ij
        noise_profile_fixed_ringdown = lambda x_lamb,a,b,c,d,e: noise_profile_lor_TLS_vec(x_lamb,a,b,c,d,tau_Q)

        #evaluated = noise_profile_lor_vec(x[index_for_fitting], a, b, c, d)
        evaluated = noise_profile_fixed_ringdown(x[index_for_fitting],a,b,c,d,tau_Q)
        evaluated = evaluated[:,0,:,:,:,:]
        ...
        fit_values = np.asarray((a_values[index1], b_values[index2], c_values[index3], d_values[index4],tau_Q))
    else:
        a_values = np.linspace(ranges[0][0], ranges[1][0], n_grid_points)
        b_values = np.linspace(ranges[0][1], ranges[1][1], n_grid_points)
        #c_values = np.linspace(ranges[0][2], ranges[1][2], n_grid_points)
        ...
        noise_profile_fixed_slope = lambda x_lamb,a,b,c,d,e: noise_profile_lor_TLS_vec(x_lamb,a,b,slope,d,tau_Q)
        evaluated = noise_profile_fixed_slope(x[index_for_fitting],a,b,slope,d,tau_Q)
        #print(evaluated)
        #print(evaluated.shape)
        evaluated = evaluated[:,0,0,:,:,:]
        data_values = np.reshape(y[index_for_fitting], (y[index_for_fitting].shape[0], 1, 1, 1))
        error = np.reshape(error, (y[index_for_fitting].shape[0], 1, 1, 1))
        ...
        fit_values = np.asarray((a_values[index1], b_values[index2], slope, d_values[index4],tau_Q))

    fit_values_names = ('a (white)', 'b (1/f)', 'c (1/f exponent)', 'd (tau)','e (tau_Q)')
    fit_result = noise_profile_lor_TLS(x, fit_values[0], fit_values[1], fit_values[2], fit_values[3],fit_values[4])
    x0_guess_result = noise_profile_lor_TLS(x, x0_guess[0], x0_guess[1], x0_guess[2], x0_guess[3],tau_Q)
    noise_slope_result = noise_slope(x, fit_values[1], fit_values[2])
    ...
    fit_dict = {'fit_values': fit_values, 'fit_values_names': fit_values_names, 'sum_dev': sum_dev,
                'fit_result': fit_result, 'x0_guess_result': x0_guess_result, 'evaluated_ranges': evaluated_ranges,
                'knee': knee,'noise_white_result': noise_white_result,
                'noise_slope_result': noise_slope_result}  # , 'x0':x0, 'z':z},'marginalized_2d':marginalized_2d,'marginalized_1d':marginalized_1d,
    return fit_dict
